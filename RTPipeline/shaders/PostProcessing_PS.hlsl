...
RWTexture2D<float4> col_acc : register(u0); //uav
RWTexture2D<float4> moment_acc : register(u1); //uav
RWTexture2D<float4> his_length : register(u2); //uav
...
Texture2D<float4> col_acc_prev : register(t8); //srv
Texture2D<float4> moment_acc_prev : register(t9); //srv
Texture2D<float4> his_length_prev : register(t10); //srv
...
float3 backProject(float x, float y, float2 res, float col_alpha, float mom_alpha)
{
	...
	float3 sample = rt_input.Load(int3(x, y, 0)).xyz;
	
	if (N <= 0 || gPosition.Load(int3(x, y, 0)).w == 0.0)
	{
		his_length[int2(x, y)] = float4(1.0, 0.0, 0.0, 0.0);
		col_acc[int2(x, y)] = float4(sample,1.0);
		moment_acc[int2(x, y)] = float4(sample.x, sample.x * sample.x, 0.0, 0.0);
		return 100.0f;
	}
	...
	float3 prevColor = float3(0.0f, 0.0f, 0.0f);
	...
	if (valid)
	{
		...
		for (int sampleIdx = 0; sampleIdx < 4; sampleIdx++)
		{
			...
			if (v[sampleIdx])
			{
				prevColor += w[sampleIdx] * col_acc_prev.Load(int3(loc, 0)).xyz;
	...
	if (!valid)
	{
		...
		for (int yy = -radius; yy <= radius; yy++)
		{
			for (int xx = -radius; xx <= radius; xx++)
			{
				...
				{
					prevColor += col_acc_prev.Load(int3(loc,0)).xyz;
	...
	if (valid)
	{
        ...
        // color accumulation
		col_acc[int2(x, y)] = float4(sample * color_alpha + prevColor * (1.0f - color_alpha), 1.0);
        // moment accumulation
		float first_moment = moment_alpha * prevMoments.x + (1.0f - moment_alpha) * sample.x;
		float second_moment = moment_alpha * prevMoments.y + (1.0f - moment_alpha) * sample.x * sample.x;
...
float4 main( float4 Position : SV_Position ) : SV_Target0
{
	...
	// shadow temporal accumulation
	//float3 color = rt_input.Load(int3(texCoord.x, texCoord.y, 0)).rgb;
	//float3 Cn_1 = col_acc_prev.Load(int3(texCoord.x, texCoord.y, 0)).rgb;
	//float3 Cn = col_alpha * color + (1.0 - col_alpha) * Cn_1;
	//col_acc[texCoord] = float4(Cn, 1);
	//return float4(Cn, 0);

	if (gPosition.Load(int3(texCoord.x, texCoord.y, 0)).w == 0.0)
	{
	...
	// testing
	//return float4(col_acc_prev.Load(int3(texCoord, 0)).xyz, 1.0);
